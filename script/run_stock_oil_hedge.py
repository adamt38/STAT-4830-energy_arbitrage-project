...
If Stooq is unreachable (no network / DNS), you can still emit aligned series for plotting::

  STAT4830_OFFLINE_HEDGE=1 python script/run_stock_oil_hedge.py --artifact-prefix week8

Offline mode sets the equity/oil sleeve to zero and ``combined`` returns equal the Polymarket sleeve.
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--artifact-prefix",
        default="week11",
        help="Prefix of processed artifacts (default week11).",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Disable VIX regime split; use only --hedge-allocation each bar.",
    )
