...
def compute_momentum_scores(
    ...
    """Compute recent signed price momentum for each token.
...
def build_dataset(
    ...
    if cfg.momentum_screening_enabled:
        quality["momentum_screening"] = {
            ...
            "selected_after_screening": len(final_market_rows),
        }
    ...
    if momentum_scores is not None:
        ...
        for row in final_market_rows:
            ...
            scored_list.append({
                ...
                "momentum": momentum_scores.get(token, 0.0),
            })
        scored_list.sort(key=lambda r: abs(float(r["momentum"])), reverse=True)
        momentum_scores_path.write_text(
            json.dumps(
                {
                    "lookback_days": cfg.momentum_lookback_days,
                    "top_n": cfg.momentum_top_n if cfg.momentum_top_n is not None else cfg.max_markets,
                    "selected_markets": scored_list,
                },
                indent=2,
            ),
            encoding="utf-8",
    ...
    if momentum_scores_path is not None:
        result["momentum_scores"] = momentum_scores_path
    return result
