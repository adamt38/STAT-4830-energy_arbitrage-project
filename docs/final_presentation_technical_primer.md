...
All figures are generated by the single Python script below. A runnable copy is checked in at [`../script/build_final_presentation_figures.py`](../script/build_final_presentation_figures.py). Run it from the repo root:

```bash
.venv/bin/python script/build_final_presentation_figures.py
```

If Matplotlib cannot write to your user cache directory, use repo-local caches:

```bash
MPLCONFIGDIR=.cache/matplotlib XDG_CACHE_HOME=.cache .venv/bin/python script/build_final_presentation_figures.py
```
...
Figures will appear in `docs/figures/final_presentation/` with names `fig01_...png` through `fig07_...png`, plus the `v*_...png` schematic and appendix figures.
