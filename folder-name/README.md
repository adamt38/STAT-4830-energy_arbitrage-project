...
- `src/equity_signal.py` (yfinance domain-proxy returns aligned to PM bars + PM/stock correlation pairs)
- `src/stock_oil_hedge.py` (optional oil + inverse-equity sleeve vs. SPY/USO; analyst CSV overlay)
- `src/cross_asset_framework.py` (IV-prior divergence, beta hedge sizing, momentum, dependency graph, VIX filter)
- `script/polymarket_week8_pipeline.py` (end-to-end run for Week 8 artifacts)
- `script/run_equity_signal.py` (standalone PM-domain/equity-sector pair feature build)
- `script/run_stock_oil_hedge.py` (fetch daily equity/oil, blend with Polymarket returns)
- `script/run_cross_asset_framework.py` (run the 5 cross-asset framework modules)
